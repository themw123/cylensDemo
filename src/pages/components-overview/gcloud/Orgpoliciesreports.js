...
const Orgpoliciesreports = () => {
    const [refreshChild, doRefreshChild] = useState(false);
    const [isLoading, setLoading] = useState(true);

    const org_policy_from_cyclens_id = (orgPolicies, cylens_id) => {
        return orgPolicies.find((org) => org.avolens_policy === cylens_id)
    }
    ...
    const ExpandableRow = ({ rowData, rowMeta }) => {
        ...
        useEffect(() => {
            util.gcp_constraints()
                .then((constrains) => constrains.find((constrain) => constrain.id === rowData[0]))
    ...
    async function getdata() {
        const org_policies = await util.gcp_constraints()
        return util
            ...
            .map(({ policy, raw }) => {
                let org_pol = org_policy_from_cyclens_id(org_policies, policy.uid)
                let to_name = (name) => ((name || "").match(/.*?constraints\/([\w-\.]+)/) || ["", ""])[1]
                // let to_name = (name) => name
                return [policy.uid, raw.metadata.name, (org_pol || {}).display_name, id_from_name((org_pol || {}).display_name), raw.spec.description];
            });
    }
...
export default Orgpoliciesreports;
